...
public class TriggerManager : BackgroundService
{
	...
	{
		// Use a deterministic ID based on the orchestration path
		var id = GenerateTriggerId(orchestrationPath);
	...
	private static string GenerateTriggerId(string orchestrationPath)
	{
		return Convert.ToHexString(
			System.Security.Cryptography.SHA256.HashData(
				System.Text.Encoding.UTF8.GetBytes(orchestrationPath)))[..16].ToLowerInvariant();
	}
	...
	private static TriggerConfig CloneTriggerConfigWithEnabled(TriggerConfig config, bool enabled)
	{
